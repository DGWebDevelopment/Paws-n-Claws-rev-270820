// 1) General initialisation of a lot of the important variables/setting up the game:
...
//2) Holding Variables to handle the game's input data:
var controlCentre= {
...
//3) Functions to control what's displayed to the players during the game:
...
//4) The following function processes who's turn it is and tells the display messages above what to say (apart from the side/name selection step at the beginning):
...
//5) Just before the game is started, the following function receives the player side/name submissions:
...
//6) Randomly pick either Paws or Claws player to go first and let them know:
...
//7) Handles the hovering over the calls by each player (greyed version until they click on it):
...
//8) Does the opposite of the hoverOn function above (reverts the cell back to the state it was before the player hovered over the cell):
...
//10) The following function processes when the relevant player actually clicks on one of the cells as the means of choosing their next play:
...
//12) The following function processes each player's input after they press the 'Submit' button
function processGo () {
...
//13) The following function calculated whether the person who has just gone has won or not (assuming they ahve at least put a number of goes in equal to the boardSize (ie. 3)):
function haveTheyWon () {
    ...
    if (controlCentre.pawsPlacements.concat(controlCentre.clawsPlacements).length==(boardSize*boardSize)) {
        alert("Nobody has won this time. Please click the OK button below to restart the game:");
        location.reload();
    };
};

//14) If a winner is decided then this function will pop up an Alert to let them know, it will then reload the page and restart the game once read:
